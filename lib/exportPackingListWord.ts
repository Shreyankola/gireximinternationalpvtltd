...
export async function exportPackingListWord(data: PackingListData) {
  const response = await fetch("/PACKING LIST DEFAULT.docx");
  ...
  // Find paragraphs by their full text (all runs combined)
  ...
  // Find paragraphs whose text STARTS with given prefix
  ...
  // Clear all <w:t> content in a paragraph, set first run to newText
  ...
  // Clear all runs in a paragraph (for values we don't need to keep)
  ...
  // Make the first run of a paragraph bold
  function makeBold(para: Element): void {
    ...
    for (let i = 0; i < all.length; i++) {
      if (all[i].localName === "r") {
        ...
        break; // only need first run
      }
    }
  }

  // Find the next paragraph after a given one
  ...
  // 1. Exporter section
  const exporterPara = findParaStarts("EXPOTER");
  if (exporterPara) {
    replaceParaText(exporterPara, `EXPOTER : ${data.exporter}`);
    makeBold(exporterPara);
    // Clear next paragraph(s) for exporter address
    let next = nextPara(exporterPara);
    let clearedCount = 0;
    while (next && clearedCount < 3) {
      const txt = getParaText(next);
      if (txt && !txt.includes("INVOICE") && !txt.includes("EXPERTER")) {
  ...
  // 3. Buyer's Order (the "-" after "Buyer's Order Number & Date :")
  ...
  // 4. Other Exporter's Details (the "-" after "Other Exporter's Details:")
  ...
  // Find the second "India" (not the one in "Apapa, Nigeria" or "Mundra, Gujarat, India")
  // Actually, the Country of Final Destination value is "India" alone in its paragraph
  // Let me check: Para 27 = "India" (alone), Para 43 = "Apapa, Nigeria", Para 49 = "Mundra, Gujarat, India", Para 100 = "India" in address
  // I need the standalone "India" which is Para 27
  // But "India" also appears in: consignee address, "Apapa, Nigeria", "Mundra, Gujarat, India", "IN INDIA" in declaration
  // Let me find it by checking surrounding context
  if (destPara) {
    const parentCell = destPara.closest("*[localName='tc']");
    // Check if there's a "Country" paragraph in the same cell or nearby
  ...
  if (consigneeLabel) {
    ...
    // Clear next consignee paragraphs
  ...
  // 8. Terms of Delivery (find "100 %ADVANCE - PARTIALLY PAYMENT ALLOWED")
  const termsPara = findParaStarts("100 %ADVANCE");
  if (termsPara) {
    replaceParaText(termsPara, data.termsOfDelivery);
    // Clear the next line "PAYMENT ALLOWED" if it exists
  ...
  // 17. Goods table - SR NO
  const srNoPara = findPara("1");
  // Find the standalone "1" that's in the goods area (not in other places like "PIN-362001,   INDIA")
  // The "1" in the goods table should be the only standalone "1"
  if (srNoPara) {
  ...
  const descPara = findParaStarts("ALUMINIUM SCRAP TENSE");
  if (descPara) {
  ...
  const pkgText = data.packagesNetGross;
  if (pkg1Para && pkg2Para) {
    replaceParaText(pkg1Para, pkgText);
    makeBold(pkg1Para);
    clearPara(pkg2Para);
  } else if (pkg1Para) {
    replaceParaText(pkg1Para, pkgText);
